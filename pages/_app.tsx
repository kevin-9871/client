...
class Application extends App<IApp> {
  ...
  static async getInitialProps({ Component, ctx }) {
    ...
    let settings = {};
    if (!process.browser) {
      const resp = await Promise.all([settingService.all(), bannerService.search({ status: 'active' })]);
      // TODO encrypt, decypt header script, footer script or other info if needed
      settings = resp[0].data;
      await updateSettingsStore(ctx, { ...settings, banners: resp[1]?.data?.data || [] });
    }
  ...
  render() {
    ...
    return (
      <Provider store={store}>
        <Head>
          <title>{typeof settings.siteName === 'string' && settings.siteName.length > 0 ? settings.siteName : 'Application'}</title>
          <meta name="viewport" content="initial-scale=1.0, width=device-width" />
        </Head>
        <Socket>
          <BaseLayout layout={layout} maintenanceMode={settings.maintenanceMode}>
            <Component {...pageProps} />
          </BaseLayout>
        </Socket>
      </Provider>
