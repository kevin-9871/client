...
const studioReducers = [
  ...
  },
  {
    ...
    reducer(state, action) {
      return {
        ...
        members: {
          ...state.members,
          data: state.members.data.map((member) => ({ ...member, status: member._id === action.payload && (member.status === 'active' ? 'inactive' : 'active') }))
        }
      };
    }
  },
  ...
  },
  {
    ...
    reducer(state, action: IReduxAction<number>) {
      const { current } = state;
      return { ...state, stats: { ...state.stats, totalPerformer: current.stats.totalPerformer + action.payload } };
    }
  },
